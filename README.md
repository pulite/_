![icon](https://github.com/pulite/_/blob/main/icon.png)
# [pulite](https://pulite.github.io/_)
## tasuraringu
japan

### puliteへようこそ!
現在このサービスはちゃんと開発中です
#### puliteとは？
2025年の初代puliteから始まった、人と人を繋ぐサービスです!  
色んな物があります。

#### 現在のpuliteシリーズ
##### 初代pulite (Original)
##### " 新・pulite (new)
##### " pulite.home (home)
##### " pulite.world (world)
*未完成の物には"が付いています
***
***
![Original pulite](https://github.com/pulite/_/blob/main/Original/logo.png)
#### [初代pulite](https://pulite.github.io/_/Original) Release / 2025
[Releaseに行く](https://github.com/pulite/_/releases#release-Original)  
puliteの始まり、初代pulite。ロイロノート・スクールの機能をフル活用し、ロイロノート内でチャットできるようにしたもの。
#### バージョン
##### ロイロノート・スクール版

1.[ファイル](https://github.com/pulite/_/releases/download/Original/Original.pulite.loilonote)をダウンロードする  
2.ロイロノート・スクールにインポートする

1.[ファイル](https://github.com/pulite/_/raw/refs/heads/main/Original/d/loilonote/pulite.loilonote)をダウンロードする  
2.ロイロノート・スクールにインポートする
***
![new pulite](https://github.com/pulite/_/blob/main/new/logo.png)
#### [新・pulite](https://pulite.github.io/_/new) / 2026
[Releaseに行く](https://github.com/pulite/_/releases#release-new)  
初めてのpuliteの次回作。標準のweb化によって、自由度が大幅に上がる。
#### バージョン
##### web版
1.[web](https://pulite.github.io/_/new)にアクセスする  
  
##### web dt版

1.```data:text/html,<script>fetch("https://raw.githubusercontent.com/pulite/_/refs/heads/main/new/index.html").then(r=>r.text()).then(h=>document.write(h))</script>```をurl欄にペーストする
